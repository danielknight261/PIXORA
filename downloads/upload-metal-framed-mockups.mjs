/**
 * Map Gelato Classic Semi-Glossy Metal Framed Poster mockups (Simple first).
 */
...
const PRODUCT_ID = "gid://shopify/Product/15939022291288";
const MOCKUP_ROOT =
  "c:\\Users\\HughesHub\\Downloads\\5261f114-5591-42a5-b462-ce43d7e614c0";
const IMAGE_ORDER = [
  "Simple.webp",
  "Close-Up-Plain-Gray-0.webp",
  "Lobby-Scandinavian-White-1.webp",
  "Bedroom-Modern-Grey-4.webp",
];
const REPORT_PATH = "downloads/metal-framed-gallery-upload-report.json";
const VARIANT_PATH = "downloads/metal-framed-variants-classed.json";
const SNIPPET_PATH = path.join(
  "apps",
  "shopify-theme",
  "snippets",
  "metal-framed-poster-gallery-data.liquid"
);
const GALLERY_JSON = "downloads/metal-framed-variant-gallery.json";

const FOLDER_RE =
  /^framed_poster_.+?_black_aluminum_w10xt22-mm_plexiglass_(.+?)_170-gsm-65lb-coated-silk_4-0_(hor|ver)$/i;
...
function matches(meta, variant) {
  // Metal framed product is black aluminum only — ignore frame option if absent
  if (variant.frame && variant.frame !== "black" && variant.frame !== "") {
    // still allow empty/unknown frame
  }
