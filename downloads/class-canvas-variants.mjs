...
function classify(v) {
  ...
  const thickness = opts.Thickness || opts.thickness || "";
  const orientationOpt = opts.Orientation || opts.orientation || "";
  const m = String(size).match(/(\d+(?:\.\d+)?)\s*[x×]\s*(\d+(?:\.\d+)?)/i);
  const square = m && Number(m[1]) === Number(m[2]);
  let orient = "portrait";
  const blob = `${orientationOpt} ${t}`.toLowerCase();
  if (blob.includes("horiz") || blob.includes("land")) orient = "landscape";
  else if (blob.includes("square")) orient = "square";
  else if (blob.includes("vert") || blob.includes("port")) orient = "portrait";
  if (square) orient = "square";
  return { orient, size, thickness, orientationOpt };
}
...
for (const r of rows) {
  ...
  byThick[r.thickness || "(none)"] = (byThick[r.thickness || "(none)"] || 0) + 1;
}
console.log({
...
fs.writeFileSync(
  "downloads/canvas-variants-classed.json",
  JSON.stringify(rows, null, 2)
);
fs.writeFileSync(
  "downloads/canvas-product-meta.json",
  JSON.stringify(
    {
      id: product.id,
      handle: product.handle,
