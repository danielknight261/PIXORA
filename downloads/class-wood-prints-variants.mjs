...
const PRODUCT_ID = "gid://shopify/Product/15942727139672";
...
function classify(v) {
  ...
  const thickRaw = String(
    opts.Thickness || opts.Depth || opts.thickness || opts.depth || ""
  );
  const thickM = thickRaw.match(/(\d+)\s*mm/i) || thickRaw.match(/^(\d+)$/);
  return {
    orient,
    size,
    sizeBlob: blob,
    ...
    thickness: thickM ? thickM[1] : "",
    frame: frameKey(opts.Frame || opts.Colour || opts.Color || opts.frame),
    assembly: opts.Assembly || "",
    orientationOpt: opts.Orientation || combined || "",
    frameOpt: opts.Frame || opts.Colour || opts.Color || opts.frame || "",
    allOpts: opts,
  };
}
...
for (const r of rows) {
  ...
  byThick[r.thickness || "(none)"] =
    (byThick[r.thickness || "(none)"] || 0) + 1;
}
console.log({
...
fs.writeFileSync(
  "downloads/wood-prints-variants-classed.json",
  JSON.stringify(rows, null, 2)
);
fs.writeFileSync(
  "downloads/wood-prints-product-meta.json",
  JSON.stringify(
    {
      id: product.id,
      handle: product.handle,
